...
def get_monthly_spending(budget_id, month):
    ...
    if response.status_code == 200:
        ...
        monthly_response = json_response["data"]["month"]
...
def get_average_spending_per_month(budget_id, start_date, end_date):
    ...
    if response.status_code == 200:
        ...
        filtered_months = [
            month for month in months_data if start_date <= month["month"] <= end_date
        ]
...
def main():
    ...
    average_spending = get_average_spending_per_month(
        MAIN_BUDGET_ID, start_of_year, end_of_year
    )

    print(
        f"Total gastado en el mes en curso ({current_month}): {format_currency(total_spending)}"
    )
